...
function cardAnalise(firstCard, secondCard) {
    picturefirstCard = firstCard.firstChild.innerHTML;
    picturesecondCard = secondCard.firstChild.innerHTML;

    if (picturefirstCard === picturesecondCard) {
        hits = hits + 2;
        console.log(hits);
        verifyGame();
    } else {
        ...
        function setWrongcard() {
            ...
            firstCard.firstChild.classList.replace('visible', 'hidden');
            secondCard.firstChild.classList.replace('visible', 'hidden');
            firstCard.lastChild.classList.replace('hidden', 'visible');
            secondCard.lastChild.classList.replace('hidden', 'visible');
        }
    }
}
